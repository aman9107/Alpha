import type { Client, Permissions, Subcommand } from "../../../client.mjs";
import Discord from "discord.js";

export default async (client: Client, message: Discord.Message<true>) => {
  (async () => {
    if (message.author.bot || message.content !== client.mention) return;
    try {
      await message.reply({
  ...
  (async () => {
    ...
    const command = client.commands.text.get(
      client.commands.aliases.has(arg) ? client.commands.aliases.get(arg)! : arg
    );
    if (!command) return;
    ...
    if (command.owner && !client.config.owners.has(message.author.id))
      try {
        await message.reply({
          content:
            "This command has been restricted to owners by the developer",
        });
      } catch {
      } finally {
        return;
    ...
    if (command.permissions || command.subcommands) {
      const missing = async (note: string, perms: string) => {
        try {
          await message.reply({
            embeds: [
      ...
      const permissions = {
        ...
        member: await (async () => {
          ...
          const channel_perms = message.channel.permissionsFor(
            message.author.id
          );
          return channel_perms ? guild_perms.add(channel_perms) : guild_perms;
        })(),
      ...
      const check_perms = async (perms: Permissions) => {
        if (perms.client) {
    ...
    try {
      ...
      console.log(
        `[command] :: '${command.name}' error :: [${client.util.time}]`
      );
