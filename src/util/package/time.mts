export default class Time extends Date {
  ...
  format(template: string, names = false) {
    ...
    const replace = (string: string, length: number) =>
      length < 0
        ? string.slice(length)
    ...
    if (typeof matched.year === "string") {
      ...
      if (isNaN(length) || length > year.length) length = year.length;
    ...
    if (typeof matched.date === "string") {
      ...
      if (isNaN(length) || length < day.length) length = day.length;
    ...
    if (typeof matched.hour === "string") {
      ...
      if (isNaN(length) || length < hour.length) length = hour.length;
    ...
    if (typeof matched.minute === "string") {
      ...
      if (isNaN(length) || length < minute.length) length = minute.length;
    ...
    if (typeof matched.second === "string") {
      ...
      if (isNaN(length) || length < second.length) length = second.length;
    ...
    if (typeof matched.ms === "string") {
      ...
      if (isNaN(length) || length < ms.length) length = ms.length;
  ...
  static seconds(timestamp: string) {
    const ms = this.ms(timestamp);
    if (typeof ms !== "number") return;
    return Math.floor(ms / 1e3);
  }

  static minutes(timestamp: string) {
    const ms = this.ms(timestamp);
    if (typeof ms !== "number") return;
    return Math.floor(ms / 6e4);
  }

  static hours(timestamp: string) {
    const ms = this.ms(timestamp);
    if (typeof ms !== "number") return;
    return Math.floor(ms / 36e5);
  }

  static days(timestamp: string) {
    const ms = this.ms(timestamp);
    if (typeof ms !== "number") return;
    return Math.floor(ms / 864e5);
  }

  static weeks(timestamp: string) {
    const ms = this.ms(timestamp);
    if (typeof ms !== "number") return;
    return Math.floor(ms / 6048e5);
  }

  static months(timestamp: string) {
    const ms = this.ms(timestamp);
    if (typeof ms !== "number") return;
    return Math.floor(ms / 2592e6);
  }

  static years(timestamp: string) {
    const ms = this.ms(timestamp);
    if (typeof ms !== "number") return;
    return Math.floor(ms / 31104e6);
  }

  static resolve(query: number | string | Date) {
    if (query instanceof Date) return new Time(query);
    if (typeof query === "number") return new Time(1970, 0, 1, 0, 0, 0, query);
  ...
  static format(template: string, date: Date, names = false) {
    if (typeof template !== "string" || !(date instanceof Date)) return;
