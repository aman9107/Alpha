...
export default {
    ...
    async execute(client, message, args) {
        const command_embed = (command) => new Discord.EmbedBuilder()
            ...
            command.permissions?.member
                ? {
                    ...
                    value: Discord.codeBlock(new Discord.PermissionsBitField()
        ...
        if (args.length > 0) {
            ...
            let result = client.commands.text.get(query);
            if (!result) {
                result = client.commands.text.reduce((total, command) => {
                    if (command.category === query)
                        total += `**${command.name}** | ${command.description?.slice(0, 20) || "No description"}\n`;
                    return total;
                }, "");
                if (result.length === 0) {
                    await message.reply({
                        embeds: [
                            client.errorEmbed.setDescription(Discord.codeBlock("No command or category is known by that name")),
                        ],
                    });
                    return;
                }
            ...
            if (!result.subcommands) {
                await message.reply({
                    embeds: [
                        client.errorEmbed.setDescription(Discord.codeBlock(`The '${query}' command has no subcommands`)),
                    ],
                });
                return;
            }
            const find_subcommand = (arg_list) => {
                const arg = arg_list.shift();
                if (typeof arg !== "string")
                    return;
                const subcommand = result.subcommands.find((i) => i.name === arg);
                if (!subcommand)
                    return;
                if (arg_list.length !== 0)
                    return find_subcommand(arg_list);
                return subcommand;
            };
            ...
            if (!subcommand) {
                await message.reply({
                    embeds: [
                        client.errorEmbed.setDescription(Discord.codeBlock("Failed to find a subcommand following the provided hierarchy")),
                    ],
                });
                return;
            }
        ...
        const categories = client.commands.text.reduce((total, command) => {
            if (total.has(command.category))
                total.get(command.category).push(command);
            else
                total.set(command.category, [command]);
            return total;
        }, new Map());
        const menu = new client.util.queue([]);
        const command_nav = () => new Discord.ActionRowBuilder().addComponents(new Discord.ButtonBuilder()
        ...
        const category_nav = (def_cat) => new Discord.ActionRowBuilder().addComponents(new Discord.StringSelectMenuBuilder()
            .setCustomId(`category_${message.author.id}`)
            .setOptions([...categories.keys()].map((category) => new Discord.StringSelectMenuOptionBuilder()
            .setLabel(category)
            .setValue(category)
